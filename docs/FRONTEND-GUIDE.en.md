...
> **Missing on purpose**: `portfolioApi` has **no** method for
> `GET /portfolios/{id}/allocation/geography` and no method for
> `GET /portfolios/{id}/allocation/sector`. The backend has served both since
> EPIC B.6/B.7; the client methods are part of the open **B.8** task
> (see chapter 11).
...
- **Dashboard** — `PortfolioLineChart` with the merged `chartData`.
...
`.classAllocation`, `transactionApi.list`, `assetApi.list`, then the session
...
- **B.8 is NOT implemented yet (issue #14)** — this is the standing state
  today. The frontend widgets for the portfolio **geography** and **sector**
  allocation do not exist:
  - `portfolioApi` has **no** `geographyAllocation(id)` /
    `sectorAllocation(id)` methods, and there are no client interfaces for the
    two responses — only `classAllocation` (`/allocation/class`, B.12) exists;
  - no `GeographyChart` / `SectorChart` components, and no dashboard/portfolio
    widget uses `/allocation/geography` or `/allocation/sector`.
  The **backend endpoints are ready** since B.6/B.7: weighted sum by the 8
  macro-regions (+ `Other`) and by the 11 GICS sectors (+ `Other`), zero-filled
  (see the backend guide, chapter 19). B.8 will add: the two client methods in
  `api.ts` (+ response types), two donut widgets on the dashboard and/or the
  portfolio page (reusing `ExposurePie` and the `ExposureRow` shape), exactly
  like the existing class-allocation donut.
...
- **The pages described in this guide are the current state**: the dashboard
  currently has no allocation donut (that is exactly what B.8 will add), and
  the old React-era description of the frontend (React 19 / axios / Recharts)
  is obsolete.