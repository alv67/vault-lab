...
| `PositionTable.svelte` (`lib/components/domain/`) | generic positions table over the `PositionRow` type (`{assetId?, ticker, name?, qty?, cost?, value?, realized?, unrealized?, roi?, closed?}`); `showCost`/`showRealized`/`showUnrealized` toggle the optional columns, `linkAssets` links the ticker to the asset page; closed rows dash out every cell except realized | the **dashboard** positions accordion (E.1) and the **portfolio detail** Positions table (E.2) |
