...
- **Language note (i18n)**: translations run on
  ...
  the other pages keep their mixed English/Italian copy
  ...
  formatters follow the same mix).
...
- Identity row: back link to `/assets`, ticker (mono font) + name, the
  ...
  and `ASSET_CLASS_LABELS` from `lib/format.ts`) and the
