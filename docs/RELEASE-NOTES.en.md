...
- Switching a chart to Table view on a phone no longer opens a scrollbar inside the card: all rows render and the page scrolls as one, and each row collapses into a compact name + figures layout that fits the screen width
