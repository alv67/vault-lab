      ...
      series: [
        {
          name: 'Gain/Loss',
          type: 'bar',
          // P/L generated inside each bucket: green bar when positive, red
          // when negative (per-bar itemStyle, mirrors `pnlColorClass`).
          data: rows.map((b) => {
            const pnl = Number(b.pnl)
            return { value: pnl, itemStyle: { color: pnl >= 0 ? semantic.positive : semantic.negative } }
          }),
        ...
        },
        {
          name: 'Realized',
          type: 'line',
          // Cumulative realized P/L at the end of each bucket.
          data: rows.map((b) => Number(b.realized)),
          smooth: false,
          symbolSize: 5,
